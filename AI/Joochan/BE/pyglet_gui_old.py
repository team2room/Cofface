...
def draw_guide_circle(image, center=None, radius=None):
    ...
    # 원 바깥 영역을 반투명하게 만들기
    alpha = 0.7  # 투명도 (0: 완전 투명, 1: 완전 불투명)
    
    # 마스크를 사용하여 원 바깥 영역을 어둡게 처리
    result_with_mask = result.copy()
    result_with_mask[mask == 0] = result_with_mask[mask == 0] * (1 - alpha)  # 원 바깥 영역 어둡게
...
class RealSenseFaceLiveness:
    def __init__(self, args):
        ...
        def on_draw():
            ...
            if self.camera_mode:
                ...
                if self.idle_gif:
                    ...
                    if current_frame:
                        ...
                        # 창 크기에 맞게 스케일 조정
                        scale_x = self.window_width / current_frame.width
                        scale_y = self.window_height / current_frame.height
                        scale = min(scale_x, scale_y)
                        self.idle_sprite.scale = scale
                        # 중앙 정렬
                        self.idle_sprite.x = (self.window_width - self.idle_sprite.width) / 2
                        self.idle_sprite.y = (self.window_height - self.idle_sprite.height) / 2
                    ...
                    if not self.idle_sprite and self.idle_image_texture:
                        ...
                        # 창 크기에 맞게 스케일 조정
                        scale_x = self.window_width / self.idle_image_texture.width
                        scale_y = self.window_height / self.idle_image_texture.height
                        scale = min(scale_x, scale_y)
                        self.idle_sprite.scale = scale
                        # 중앙 정렬
                        self.idle_sprite.x = (self.window_width - self.idle_sprite.width) / 2
            ...
            # 모드 표시
            self.mode_label.text = f'모드: {"카메라" if self.camera_mode else "대기"} (C 키로 전환)'
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == pyglet.window.key.Q:
            ...
            print(f"모드 전환: {'카메라' if self.camera_mode else '대기'}")
    ...
    def update(self, dt):
        ...
        try:
            ...
            if not self.initialization_done and self.color_texture is None and self.camera_mode:
                # 초기화 중이면 원본 이미지 표시
        ...
        while True:
            try:
                ...
                for i, face in enumerate(faces):
                    ...
                    # 나이 및 성별 추정
                    age = face.age if hasattr(face, 'age') else 0
                    gender = face.gender if hasattr(face, 'gender') else 0
                    ...
                    # 콘솔에 얼굴 정보 표시
                    gender_str = "남성" if gender == 1 else "여성"
                    print(f"얼굴 #{i+1} - 라이브니스: {'실제' if is_live else '가짜'}, 나이: {age:.1f}세, 성별: {gender_str}")
