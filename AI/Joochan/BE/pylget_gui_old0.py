...
class RealSenseFaceLiveness:
    def __init__(self, args):
        ...
        # 가림막 생성 (검은색 배경)
        mask_image = np.ones((self.window.height, self.window.width, 3), dtype=np.uint8) * 18  # 다크 모드 배경색
        ...
        if self.mask_texture:
            ...
            self.mask_sprite.scale_x = self.window.width / self.mask_texture.width
            self.mask_sprite.scale_y = self.window.height / self.mask_texture.height
        ...
        def on_draw():
            ...
            if self.camera_mode:
                ...
                if self.idle_gif:
                    ...
                    if self.idle_sprite:
                        self.idle_sprite.draw()
                elif self.color_sprite:
                    # 대기 모드지만 카메라 화면 표시 (얼굴 인식 없이)
                    self.color_sprite.draw()
                else:
                    # 기본 대기 화면 표시 (검은 배경)
                    if self.mask_sprite:
    ...
    def update(self, dt):
        ...
        try:
            ...
            # 가이드 원 적용 (카메라 모드에서만 사용)
            if self.camera_mode and self.guide_circle:
                center = (color_image.shape[1] // 2, color_image.shape[0] // 2)
                radius = min(color_image.shape[1], color_image.shape[0]) // 4
                color_image = draw_guide_circle(color_image, center, radius, bg_color=(18, 18, 18))  # 다크 모드 색상
            ...
            # 항상 비디오 프레임 업데이트 (대기 모드에서도 카메라 화면 업데이트)
            ...
            if self.color_texture:
                ...
                # 전체 윈도우 크기에 맞게 스케일 조정
                scale_x = self.window.width / self.color_texture.width
                scale_y = self.window.height / self.color_texture.height
            ...
            if self.camera_mode and self.show_depth:
                # 깊이 텍스처 업데이트
            ...
            # 카메라 모드일 때만 얼굴 인식 처리 큐에 추가
            if self.initialization_done and (self.camera_mode or self.processing_api_request):
                try:
                    # 얼굴 검출을 위한 원본 이미지 저장
    ...
    def process_frames(self):
        ...
        while True:
            try:
                ...
                # 대기 모드일 때는 프레임 큐 확인만 하고 처리하지 않음
                ...
                # 가장 가까운 얼굴만 처리 (깊이가 가장 낮은 얼굴)
                if valid_faces:
                    # 깊이에 따라 정렬 (작은 값이 가까운 것 = 낮을수록 가까움)
                    ...
                    if self.save_embeddings and is_live:
                        ...
                        filename = self.save_embedding(embedding_3d, is_live, age, gender, timestamp)
                    ...
                    # 랜드마크 그리기 제거 - 노란색 점과 녹색 사각형 표시 안 함
