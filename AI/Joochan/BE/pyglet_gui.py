...
def crop_to_aspect_ratio(image, aspect_ratio_width=12, aspect_ratio_height=17):
    """
    원하는 종횡비에서 살짝 여유를 줘서 이미지 중앙을 기준으로 잘라냄
    """
    h, w = image.shape[:2]
...
class RealSenseFaceLiveness:
    def __init__(self, args):
        ...
        from pyglet.image import load as pyglet_image_load

        self.overlay_sprite = None
        overlay_path = "mask_overlay.png"

        try:
            overlay_img = pyglet_image_load(overlay_path)
            self.overlay_sprite = pyglet.sprite.Sprite(overlay_img, x=0, y=0)
            print(f"[Overlay] 오버레이 PNG 로드 성공: {overlay_path}")
        except Exception as e:
                ...
                if self.camera_mode:
                    ...
                    if self.color_sprite:
                        ...
                        if self.overlay_sprite:
                            scale_x = self.window.width / self.overlay_sprite.image.width
                            scale_y = self.window.height / self.overlay_sprite.image.height
                            scale = min(scale_x, scale_y)

                            self.overlay_sprite.scale = scale

                            # 중앙 정렬
                            self.overlay_sprite.x = (self.window.width - self.overlay_sprite.width) / 2
                            self.overlay_sprite.y = (self.window.height - self.overlay_sprite.height) / 2
    ...
    def initialize_face_app(self):
        ...
        try:
            ...
            self.face_app = FaceAnalysis(
                ...
                allowed_modules=['detection', 'recognition', 'genderage'],  # 명시적으로 성별/나이 모듈 활성화
                providers=['CUDAExecutionProvider', 'CPUExecutionProvider']
            )
            self.face_app.prepare(ctx_id=self.gpu_id, det_size=(self.width, self.height))
            
            # 모델 로드 상태 확인
            print(f"FaceAnalysis 로드된 모듈: {self.face_app.models}")
        ...
        def capture_loop():
            while True:
                try:
                    ...
                    if self.rotation != 0:
                        original_color = rotate_image(original_color, self.rotation, False)  # mirror already applied
                        original_depth = rotate_image(original_depth, self.rotation, False)
                    
                    # Create copies for display
                    color_image = original_color.copy()
                    depth_image = original_depth.copy()
                    
                    # Get crop dimensions before cropping
                    h, w = color_image.shape[:2]
                    
                    # Apply aspect ratio cropping to both display and detection images
                    color_image = crop_to_aspect_ratio(color_image)
                    depth_image = crop_to_aspect_ratio(depth_image)
                    
                    # Calculate crop offsets (needed to adjust face detection coordinates later)
                    cropped_h, cropped_w = color_image.shape[:2]
                    offset_x = (w - cropped_w) // 2
                    offset_y = (h - cropped_h) // 2
                    
                    # Apply same cropping to the original images used for detection
                    original_color = crop_to_aspect_ratio(original_color)
                    original_depth = crop_to_aspect_ratio(original_depth)

                    if self.initialization_done:
                        while not self.frame_queue.empty():
                            self.frame_queue.get_nowait()
                        # Store the cropped originals and the display images, along with crop offsets
                        self.frame_queue.put_nowait((
                            original_color, original_depth, 
                            color_image.copy(), depth_image.copy(),
                            offset_x, offset_y
                        ))
    ...
    def create_api_result(self):
        ...
        # 결과 요약 로그
        print(f"API 결과 집계: 나이 샘플={len(ages)}개, 성별 샘플={len(genders)}개")
        print(f"평균 나이: {avg_age:.1f}세, 최빈 성별: {gender}")
                ...
                if valid_faces:
                    ...
                    try:
                        ...
                        if gender not in [0, 1]:
                            print(f"경고: 비정상적인 성별 값 감지 ({gender}), 기본값 1(남성)으로 설정")
                            gender = 1
                    except Exception as e:
                        print(f"나이/성별 처리 중 오류 발생: {e}")
                        age = 30.0
                        gender = 1
