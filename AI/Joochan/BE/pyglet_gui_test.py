            ...
            try:
                ...
                if self.camera_mode:
                    ...
                    if self.color_sprite:
                        ...
                        # PNG 오버레이 그리기 (가이드 원 대신)
                        current_overlay = self.overlay_sprite if self.face_detected else self.overlay_no_face_sprite
                        if current_overlay:
                ...
                if self.show_ui:
                    ...
                    if self.status_label:
                        ...
                        self.status_label.text = f'상태: {status_text} | 점수: {"켜짐" if self.show_scores else "꺼짐"} | 깊이: {"켜짐" if self.show_depth else "꺼짐"} | 반전: {mirror_status} | 범위: {self.min_depth}~{self.max_depth}m | {frame_skip_info}'
    ...
    def initialize_face_app(self):
        try:
            ...
            self.face_app = FaceAnalysis(
                ...
                allowed_modules=['detection', 'recognition']  # genderage 제외
            )
            self.face_app.prepare(ctx_id=self.gpu_id, det_size=(self.width, self.height))

            # genderage 모델 직접 로드
            import onnxruntime as ort
            print("Gender/Age 모델 직접 로드 중...")
            genderage_path = str(inface_models / "models" / "buffalo_l" / "genderage.onnx")
            self.genderage_session = ort.InferenceSession(
                genderage_path,
                providers=['CUDAExecutionProvider']
            )
            print("Gender/Age 모델 로드 완료")
    ...
    def process_frames(self):
        ...
        while True:
            try:
                ...
                # 프레임 스킵 로직: frame_skip 간격으로 처리
                process_this_frame = (self.frame_counter % self.frame_skip == 0) or self.processing_api_request
                ...
                if process_this_frame:
                    ...
                    for face in faces:
                        ...
                        if is_valid_depth and self.min_depth <= avg_depth <= self.max_depth:
                            # 추가: genderage 모델 직접 사용
                            try:
                                bbox_int = bbox.astype(int)
                                x1, y1, x2, y2 = bbox_int
                                if (x2 - x1) > 0 and (y2 - y1) > 0:
                                    # 얼굴 이미지 추출
                                    face_img = original_color[y1:y2, x1:x2]
                                    ...
                                    # 추론
                                    input_name = self.genderage_session.get_inputs()[0].name
                                    outputs = self.genderage_session.run(None, {input_name: face_img})

                                    # 결과 저장
                                    age = float(outputs[0][0][0] * 100)  # 0-1 → 0-100
                                    gender_logit = float(outputs[1][0][0])
                                    gender = 1 if gender_logit > 0 else 0
                    ...
                    if valid_faces:
                        ...
                        # 안전한 문자열 포맷팅 (None 방지)
                        age_str = f"{age:.1f}" if age is not None else "알 수 없음"
                        faces_results = [(rotated_bbox, is_live, liveness_scores, age, gender)]
                        # 마지막 성공한 결과 저장
                        last_faces_results = faces_results.copy()

                        print(
                            f"얼굴 감지: 라이브니스: {'실제' if is_live else '가짜'}, 깊이: {closest_depth:.2f}m, 나이: {age_str}세, 성별: {gender_str}")
                    ...
                    if frame_count >= 10:
                        ...
                        self.processing_fps = frame_count / (
                            time.time() - start_time if time.time() - start_time > 0 else 0.001)
        ...
        try:
            ...
            try:
                with self.app_instance.frame_queue.mutex:
                    if self.app_instance.frame_queue.queue:
                        # (original_color, original_depth, rotated_color, rotated_depth)
            ...
            try:
                ...
                if color_frame:
                    ...
                    # 이미지 전처리 (필요시)
                    current_color = rotate_image(current_color, self.app_instance.rotation, self.app_instance.mirror)
