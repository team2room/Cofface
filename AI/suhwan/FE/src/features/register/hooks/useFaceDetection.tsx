...
export function useFaceDetection() {
  ...
  const startVideo = async (): Promise<void> => {
    ...
    try {
      ...
      // 크롭 및 줌인 효과를 위한 이미지 처리 함수 설정
      const processFrameWithCrop = () => {
        ...
        // 크롭 중심점과 크기 설정 (줌인 효과를 위해 원본보다 작게 크롭)
        const cropFactor = 0.6 // 60%만 사용하여 40% 줌인 효과
        const cropWidth = videoWidth * cropFactor
        const cropHeight = videoHeight * cropFactor
        ...
        // 애니메이션 프레임 요청
        requestAnimationFrame(processFrameWithCrop)
      }
  ...
  const handleComplete = async (): Promise<void> => {
    ...
    try {
      // 랜덤 사용자 ID 생성 (실제로는 백엔드에서 생성)
      const tempUserId = `user_${Date.now().toString().slice(-6)}`

      // 서버에 얼굴 등록 API 호출
      const result = await registerFace(tempUserId, capturedImages)
      ...
      navigate('/register/face/confirm', {
        state: {
          userId: result.user_id || tempUserId, // 서버에서 받은 ID 또는 임시 ID
