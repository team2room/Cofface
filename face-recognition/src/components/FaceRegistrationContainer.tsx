...
const FaceRegistrationContainer: React.FC = () => {
  ...
  const [currentAngle, setCurrentAngle] = useState<{yaw: number, pitch: number, roll: number}>({yaw: 0, pitch: 0, roll: 0});
  ...
  useEffect(() => {
    ...
    moduleRef.current = new FaceRegistrationModule({
      angleThreshold: 10, // 더 유연한 각도 허용 범위
      imageQuality: {
    ...
    const initialize = async () => {
      try {
        ...
        if (!isInitialized) {
          throw new Error('얼굴 인식 모듈 초기화에 실패했습니다.');
        }
      ...
      if (
        ...
        try {
          ...
          if (faceInfo) {
            ...
            // 기본 가이드라인 그리기
            drawDefaultGuide(ctx, canvas.width, canvas.height);
          }
        } catch (error) {
      ...
      <div className="face-registration-container complete">
        ...
        <div className="captured-images-container">
          {DIRECTIONS.map(direction => 
            capturedImages[direction] && (
              <div key={direction} className="captured-image">
                <h3>{direction}</h3>
                <img 
                  src={capturedImages[direction]} 
                  alt={`${direction} 방향 얼굴`} 
                  width="120" 
                />
              </div>
        ...
        <div className="angle-info">
          <div className="angle-display">
            <div className="angle-value">
              <span>Yaw: </span>
              <strong>{currentAngle.yaw.toFixed(1)}°</strong>
            </div>
            <div className="angle-value">
              <span>Pitch: </span>
              <strong>{currentAngle.pitch.toFixed(1)}°</strong>
            </div>
            <div className="angle-value">
              <span>Roll: </span>
              <strong>{currentAngle.roll.toFixed(1)}°</strong>
            </div>
          </div>
        </div>
      </div>
