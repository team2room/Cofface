...
const FaceRecognition: React.FC = () => {
  ...
  const [stateStable, setStateStable] = useState<boolean>(false);
  const [rotation, setRotation] = useState<RotationState>({ roll: 0, pitch: 0, yaw: 0 });
...
const onResults = (results: mp.Results): void => {
  ...
  if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
    ...
    drawing.drawConnectors(
      canvasCtx, landmarks, mp.FACEMESH_TESSELATION,
      { color: 'rgba(180, 180, 180, 0.5)', lineWidth: 1 }
    );
    
    drawing.drawConnectors(
      canvasCtx, landmarks, mp.FACEMESH_FACE_OVAL,
    ...
    const currentState = detectionState === FaceDetectionState.INIT ? 
    ...
    console.log('상태 확인:', {
      ...
      isFaceInCircle,
      isDirectionCorrect,
      roll: rotationValues.roll.toFixed(1),
      pitch: rotationValues.pitch.toFixed(1),
      yaw: rotationValues.yaw.toFixed(1),
      processing,
      stateStable
    });
    ...
    if (currentState !== FaceDetectionState.INIT) {
      checkFaceOrientation(rotationValues, isFaceInCircle, faceDetectedNow, currentState);
    }
  } else {
    ...
    canvasCtx.fillRect(canvasElement.width / 2 - 150, canvasElement.height / 2 - 20, 300, 40);
    canvasCtx.fillStyle = 'red';
    canvasCtx.font = '22px "Noto Sans KR", sans-serif';
    canvasCtx.textAlign = 'center';
    canvasCtx.fillText("얼굴이 감지되지 않았습니다", canvasElement.width / 2, canvasElement.height / 2 + 7);
  }
  
  // 가이드라인 그리기 (얼굴 원 위치 표시)
  canvasCtx.strokeStyle = faceWithinBounds ? 'rgba(0, 200, 83, 0.5)' : 'rgba(255, 171, 0, 0.5)';
  ...
  const checkFaceInCircle = (landmarks: mp.NormalizedLandmarkList): boolean => {
    ...
    const distance = Math.sqrt(
      Math.pow(nose.x - center.x, 2) + 
      Math.pow(nose.y - center.y, 2)
    );
  ...
  // 얼굴 회전 계산 함수 (0도 부근으로 보정)
  const calculateFaceRotation = (landmarks: mp.NormalizedLandmarkList): RotationState => {
  ...
  const updateDebugCanvas = (rotationValues: RotationState): void => {
    ...
    // 각도 값 표시 (roll, pitch, yaw)
    ctx.font = '12px monospace';
    ctx.textAlign = 'left';
    
    // Roll (Z축 회전)
    ctx.fillStyle = '#FF8080';
    ctx.fillText(`Roll: ${rotationValues.roll.toFixed(1)}°`, 10, 35);
    const rollStatus = Math.abs(rotationValues.roll) < 15 ? 'OK' : 'NG';
    ctx.fillText(rollStatus, canvas.width - 30, 35);
    
// Pitch (X축 회전)
ctx.fillStyle = '#80FF80';
ctx.fillText(`Pitch: ${rotationValues.pitch.toFixed(1)}°`, 10, 55);

let pitchStatus = 'NG';
if (detectionState === FaceDetectionState.UP_FACE && rotationValues.pitch < -25) {
  pitchStatus = 'OK';
} else if (detectionState === FaceDetectionState.DOWN_FACE && rotationValues.pitch > 25) {
  pitchStatus = 'OK';
...
ctx.fillText(`Yaw: ${rotationValues.yaw.toFixed(1)}°`, 10, 75);

let yawStatus = 'NG';
if (detectionState === FaceDetectionState.LEFT_FACE && rotationValues.yaw > 25) {
  yawStatus = 'OK';
} else if (detectionState === FaceDetectionState.RIGHT_FACE && rotationValues.yaw < -25) {
  yawStatus = 'OK';
...
ctx.font = '12px monospace';
ctx.textAlign = 'center';
ctx.fillText(`현재 상태: ${FaceDetectionState[detectionState]} (${detectionState}/6)`, canvas.width / 2, 95);
...
// 얼굴 윤곽 그리기
ctx.beginPath();
ctx.ellipse(0, 0, radius * yawFactor, radius * pitchFactor, 0, 0, 2 * Math.PI);
...
const noseEndY = noseLength * Math.sin(rotationValues.pitch * Math.PI / 180);
...
const isCorrectOrientation = (rotation: RotationState, state: FaceDetectionState): boolean => {
  // 디버그 정보 출력
  console.log('방향 체크 상세 값:', {
    state: FaceDetectionState[state],
    roll: rotation.roll.toFixed(1),
    pitch: rotation.pitch.toFixed(1),
    yaw: rotation.yaw.toFixed(1)
  });
  ...
  switch (state) {
    case FaceDetectionState.FRONT_FACE:
      // 정면 상태일 때는 임시로 항상 true 반환
      console.log('정면 상태에서 자동 통과');
      return true;
      
    case FaceDetectionState.LEFT_FACE:
      // 왼쪽: yaw가 25~45도, pitch는 ±15도 이내
      ...
      result = leftRollOK && leftPitchOK && leftYawOK;
    ...
    case FaceDetectionState.RIGHT_FACE:
      ...
      result = rightRollOK && rightPitchOK && rightYawOK;
    ...
    case FaceDetectionState.UP_FACE:
      ...
      result = upRollOK && upPitchOK && upYawOK;
    ...
    case FaceDetectionState.DOWN_FACE:
      ...
      result = downRollOK && downPitchOK && downYawOK;
...
if (!stateStable) {
  console.log("상태가 안정화되지 않아 타이머 미작동");
  return;
}
...
const getStateLabel = (state: FaceDetectionState): string => {
  switch (state) {
    case FaceDetectionState.FRONT_FACE: return "정면";
    case FaceDetectionState.LEFT_FACE: return "좌측";
    case FaceDetectionState.RIGHT_FACE: return "우측";
    case FaceDetectionState.UP_FACE: return "위";
    case FaceDetectionState.DOWN_FACE: return "아래";
    default: return "";
  }
};
...
return (
  <ColorGuide>
    ...
    <ColorItem>
      ...
      <div>
        <strong>주황색</strong>: 얼굴은 원 안에 있으나<br/>방향이 올바르지 않음
      </div>
    </ColorItem>
    ...
    <ColorItem>
      ...
      <div>
        <strong>초록색</strong>: 위치와 방향 모두 올바름<br/>(촬영 준비됨)
      </div>
    </ColorItem>
...
return (
  <ColorGuide>
    ...
    <div style={{ marginBottom: '10px' }}>
      ...
      {detectionState === FaceDetectionState.FRONT_FACE && '정면 촬영 중 (1/5)'}
      {detectionState === FaceDetectionState.LEFT_FACE && '왼쪽 촬영 중 (2/5)'}
      {detectionState === FaceDetectionState.RIGHT_FACE && '오른쪽 촬영 중 (3/5)'}
      {detectionState === FaceDetectionState.UP_FACE && '위쪽 촬영 중 (4/5)'}
      {detectionState === FaceDetectionState.DOWN_FACE && '아래쪽 촬영 중 (5/5)'}
    ...
    <CameraColumn>
      {detectionState !== FaceDetectionState.COMPLETED && (
        <FaceCircle borderColor={borderColor}>
          ...
          {/* 완료 화면 - 캡처된 이미지들 */}
          {detectionState === FaceDetectionState.COMPLETED && renderCapturedImages()}
          ...
          {detectionState === FaceDetectionState.COMPLETED && (
            <Button onClick={handleRestartCapture}>
              다시 촬영하기
            </Button>
          )}
        ...
        <InfoColumn>
          ...
          detectionState !== FaceDetectionState.COMPLETED && (
            <DebugPanel>
              ...
              <div style={{ borderBottom: '1px solid #555', paddingBottom: '5px', marginBottom: '10px' }}>
                <strong>현재 정보</strong>
              </div>
              
              <DebugValue>
                <span>현재 상태:</span>
                <span>{FaceDetectionState[detectionState]} ({detectionState})</span>
              </DebugValue>
              ...
              <div style={{ borderBottom: '1px solid #555', paddingBottom: '5px', margin: '10px 0' }}>
                <strong>회전 값</strong>
              </div>
              
              <DebugValue>
                <span>Roll (Z축):</span>
                <span>{rotation.roll.toFixed(1)}°</span>
              </DebugValue>
              <DebugValue>
                <span>Pitch (X축):</span>
                <span>{rotation.pitch.toFixed(1)}°</span>
              </DebugValue>
              <DebugValue>
                <span>Yaw (Y축):</span>
                <span>{rotation.yaw.toFixed(1)}°</span>
              </DebugValue>
