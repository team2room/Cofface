...
// 타입과 유틸 함수 임포트
import { FaceDetectionState, RotationState, CapturedImage } from './types'
import {
  checkFaceInCircle,
...
const FaceRecognition: React.FC = () => {
  ...
  useEffect(() => {
    if (detectionState === FaceDetectionState.COMPLETED) {
      ...
      console.log(
        '캡처된 이미지 상태:',
        capturedImages.map(
          (img) => `${FaceDetectionState[img.state]} (${img.state})`,
        ),
      )
    }
  ...
  useEffect(() => {
    ...
    if (detectionState !== FaceDetectionState.INIT) {
      ...
      console.log(
        `UI 업데이트: ${getMessage(detectionState, loadingError, modelsLoaded)} / ${getSubMessage(detectionState, loadingError)}`,
      )
    }
