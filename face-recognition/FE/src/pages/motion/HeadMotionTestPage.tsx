...
export const HeadMotionTestPage: React.FC = () => {
  ...
  const handleMotionDetected = (motionType: string, data: any) => {
    ...
    if (currentMission) {
      ...
      // 미션과 감지된 모션 비교
      if (
        (currentMission === '왼쪽으로 고개 돌리기' && motionType === 'QUICK_LEFT_TURN') ||
        (currentMission === '오른쪽으로 고개 돌리기' && motionType === 'QUICK_RIGHT_TURN') ||
        (currentMission === '고개 좌우로 흔들기' && motionType === 'HEAD_SHAKE') ||
  ...
  return (
    <Container>
      <Header>
        ...
        <Button onClick={() => navigate('/motion')}>기본 모드로 돌아가기</Button>
      </Header>
        ...
        <LeftPanel>
          ...
          {!currentMission && (
            <div style={{ marginTop: '20px', textAlign: 'center' }}>
              <Button onClick={startRandomMission} style={{ 
                padding: '15px 30px',
                fontSize: '16px',
            ...
            {testResults.length === 0 ? (
              ...
              testResults.map((result, index) => (
                <TestResult key={index} success={result.success}>
                  <div><b>감지된 모션:</b> {result.type}</div>
                  <div><b>결과:</b> {result.success ? '성공' : '실패'}</div>
                  <div><b>시간:</b> {result.timestamp}</div>
                </TestResult>
              ))
            )}
          </TestResultSection>
