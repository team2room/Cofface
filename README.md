...
- **얼굴등록** : 사용자(앱) → 카메라 → 얼굴 캡처 → 5방향 촬영 완료
  → 서버로 이미지 전송 → 벡터 임베딩 생성 → Qdrant DB 저장

- **얼굴인식** : 사용자(키오스크) → 카메라 → 이미지 캡처 → 라이브니스 검사(depth 카메라)
  → GPU 서버로 이미지 전송 → 임베딩 생성 → Qdrant DB 검색
  → 유사도 계산 → 결과 반환 → 인증 완료/실패

- **모션감지** : 사용자(키오스크) → MediaPipe FaceMesh → 얼굴 랜드마크 추출
→ 3축 회전 계산(pitch, yaw, roll) → 모션 패턴 분석 → 모션 이벤트 발행
→ 구독 컴포넌트에서 이벤트 처리 → UI 업데이트/액션 수행
</details>

<details>
<summary><strong>FCM</strong></summary>
...
<details>
<summary><strong>추천</strong></summary>
