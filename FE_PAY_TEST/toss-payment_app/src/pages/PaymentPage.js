...
const TOSS_PAYMENTS_TEST_CLIENT_KEY =
...
const PaymentPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchClientKey = async () => {
      try {
        ...
        if (response.success) {
          setClientKey(
            response.data.clientKey || TOSS_PAYMENTS_TEST_CLIENT_KEY
          );
        } else {
  ...
  useEffect(() => {
    ...
    const loadWidget = async () => {
      try {
        const paymentWidgetInstance = await loadPaymentWidget(
          clientKey,
          'test'
        );
    ...
    cart.forEach((item) => {
      ...
      if (selectedOptions[item.id]) {
        selectedOptions[item.id].forEach((optionId) => {
          const option = TEST_OPTIONS[item.id].find(
            (opt) => opt.id === optionId
          );
          if (option) {
    ...
    try {
      ...
      const menuOrders = cart.map((item) => ({
        ...
        options: selectedOptions[item.id]
          ? selectedOptions[item.id].map((optionId) => ({
            ...
            {TEST_PRODUCTS.map((product) => (
              <div key={product.id} className="menu-item">
                <div className="menu-image">
                  <img
                    src={product.image || '/images/default-menu.jpg'}
                    alt={product.name}
                  />
                </div>
                ...
                <div key={item.id} className="cart-item">
                  ...
                  <div className="cart-item-options">
                    ...
                    <div className="options-list">
                      {TEST_OPTIONS[item.id] &&
                        TEST_OPTIONS[item.id].map((option) => (
